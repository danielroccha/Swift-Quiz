...
class QuizManager {
    ...
    private var _totalHitAnswers = 0
    ...
    var totalHitAnswers: Int {
        return _totalHitAnswers
    }
    
    func refreshQuiz() {
        let randomIndex = Int(arc4random_uniform(UInt32(quizes.count)))
        let quizData = quizes[randomIndex]
        quiz = Quiz(question: quizData.question, answers: quizData.options, correctAnswer: quizData.correctAnswer)
    }
    
    func validateAnswer(index:Int)  {
        _totalAnswers += 1
        if quiz.validateAnswer(index) {
            _totalHitAnswers += 1
        }
    }
